...
void initEkKeys()
{
    ...
    /**
     * We directly use the EK template generated by code here,
     * instead of building the EK template from the data in the NV indices.
     * This is critical for the security, because the FWIDs only attest the code,
     * but not the data in the NV indices. Thus, the template can be affected
     * from ordinary TPM usage by writing data to the EK template NV indices.
     * By directly using the EK template generated by code,
     * we allow the verifier to implicitly derive that the EK is restricted,
     * by trusting the FWID. Assuming the verifier only trusts a fTPM with a FWID that generates a restricted EK.
     */
    result = GetSigningEkTemplate(&publicArea);
    if (result != TPM_RC_SUCCESS)
    {
        EMSG("GetSigningEkTemplate failed with 0x%02x", result);
        return;
    }
